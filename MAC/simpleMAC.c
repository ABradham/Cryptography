...
void des_decrypt(unsigned char block[8], unsigned char key[8], unsigned char output[8]) {
    ...
    unsigned char subkeys[16][6]; // TURNED INTO AN ARRAY
 ...
 // Calculate 16 Rounds of the cipher
    ...
    for (int i=15; i>=0; i -=2) {
        ...
        des_feistel(right_block, subkeys[i], fiestel_output);
        xor(fiestel_output, left_block, left_block, 4);


        des_feistel(left_block, subkeys[i-1], fiestel_output);
...
void des_encrypt(unsigned char block[8], unsigned char key[8], unsigned char output[8]) {
    ...
    // In the original code, each loop iteration calculates two rounds.  This way there were no
    // memcoppies at the end of each round to for example switch right_block and left_block.

    // sws rewrote to make 16 explicit iterations, to potentially help with HW1Grad
...
/**
 * A function to implement CBC-MAC with the DES block cipher.
 * @param blocks: The plaintext blocks (8 bytes apiece) you want to compute a MAC for
 * @param num_blocks: The number of blocks (8 byte chunks) in your message / plaintext
 * @param key: The key to use for DES
 * @param output: A pointer representing where you want the output MAC to go
*/
void cbc_des_mac(unsigned char* blocks, unsigned int num_blocks, unsigned char key[8], unsigned char* output){
    // Res starts as 0 since A ^ 0 = A
    unsigned char res[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

    for(int i = 0; i < num_blocks; i++){
        // XOR previous res with current plaintext message
        unsigned char curr_res[8];
    ...
    memcpy(output, res, 8 * sizeof(unsigned char));
}

/**
 * A function to calcualte a collision between the CBC-MAC of M_2 
 * (M_1 is neccesary because this attack requires knowledge of 2 plaintext-MAC pairs)
 * @param m_1: The first message
 * @param m_2: The second message
*/
void calc_collision(unsigned char* m_1, unsigned char* m_2, unsigned int m_1_blocks, unsigned int m_2_blocks, unsigned char key[8]){
    // Calculate the hash of m_1 and m_2
    unsigned char m_1_MAC[8];
    unsigned char m_2_MAC[8];
    cbc_des_mac(m_1, m_1_blocks, key, m_1_MAC);
    cbc_des_mac(m_2, m_2_blocks, key, m_2_MAC);
    
    // Construct middle injection from m_1_hash XOR (first 8 bytes of M_2)
    unsigned char injection[8];
    xor(m_1_MAC, m_2, injection, 8);

    // Construct final message to hash (M1 || MAC(M_1) XOR (first 8 bytes) M_2 || Remaining bytes of M2)
    unsigned int colliding_message_length = (8 * m_1_blocks) + 8 + ((8*m_2_blocks) - 8);
    unsigned char colliding_message[colliding_message_length];
    memcpy(colliding_message, m_1, m_1_blocks * 8 * sizeof(unsigned char));
    memcpy(&colliding_message[m_1_blocks * 8], injection, 8 * sizeof(unsigned char));
    if(m_2_blocks > 1)
        memcpy(&colliding_message[(8*m_1_blocks) + 8], &m_2[8], 8 * (m_2_blocks - 1));

    // Calculate MAC of original messages
    printf("M_1: %s\n", bin_to_string(m_1, 8 * m_1_blocks));
    printf("M_2: %s\n\n", bin_to_string(m_2, 8 * m_2_blocks));
    printf("H_1: %s\n", bin_to_string(m_1_MAC, 8));
    printf("H_2: %s\n\n", bin_to_string(m_2_MAC, 8));
    
    // Show contrived message has same MAC as M_2
    unsigned char colliding_message_MAC[8];
    cbc_des_mac(colliding_message, m_1_blocks + m_2_blocks, key, colliding_message_MAC);
    printf("Collision MAC: %s\n\n", bin_to_string(colliding_message_MAC, 8));
